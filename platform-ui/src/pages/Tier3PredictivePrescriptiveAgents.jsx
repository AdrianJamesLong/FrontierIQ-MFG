...
import { Brain, Lightbulb, FileText, Target, TrendingUp, Award, Zap, Send, Bot, User, Loader2, Sparkles, History, Plus, Trash2, X, MessageSquare, Activity, Database, Server, CheckCircle, Clock, Info } from 'lucide-react'
...
function Tier3PredictivePrescriptiveAgents() {
  const [activeTab, setActiveTab] = useState('operations-recommendation')
  const [messages, setMessages] = useState({
    'operations-recommendation': [{
      role: 'assistant',
      content: 'I am the Operations Recommendation Agent for NovaChem Grangemouth. I orchestrate the Performance Analyst, Line Operations, Downtime RCA, and Throughput RCA agents to produce ranked, actionable improvement recommendations. Ask me about the top actions to improve throughput, where to focus this week, or the highest-impact opportunities on the plant.',
      timestamp: new Date(),
      auditTrail: []
    }],
    'executive-briefing': [{
      role: 'assistant',
      content: 'I am the Executive Briefing Agent for NovaChem Grangemouth. I coordinate specialist agents across performance, operations, and root cause analysis to produce concise, plain-language summaries for plant leadership. Ask me to summarise yesterday for the plant director, flag what leadership should be concerned about, or confirm whether we are on track for monthly targets.',
      timestamp: new Date(),
      auditTrail: []
    }]
  })
  ...
  const loadChatThreads = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/agent/chat-threads/tier3`)
      if (response.ok) {
  ...
  const loadChatThread = async (threadId) => {
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        const rawType = data.agent_type || `tier3_${activeTab}`
        const agentType = rawType.replace('tier3_', '')
        const validTab = ['operations-recommendation', 'executive-briefing'].includes(agentType) ? agentType : activeTab
        setMessages(prev => ({
          ...prev,
  ...
  const startNewChat = () => {
    setMessages({
      'operations-recommendation': [{
        role: 'assistant',
        content: 'Hello! I\'m the Operations Recommendation Agent. I suggest actions, not commands. I consume outputs from Analyst + RCA agents to rank improvement opportunities, estimate impact, and explain trade-offs. Ask me about improvement opportunities, where to focus maintenance, or the biggest wins available.',
        timestamp: new Date(),
        auditTrail: []
      }],
      'executive-briefing': [{
        role: 'assistant',
        content: 'Hello! I\'m the Executive Briefing Agent. I translate factory reality to leadership by creating exec-ready briefs, highlighting risks & wins, and avoiding technical detail. Ask me for leadership summaries, what to worry about, or if we\'re on track.',
        timestamp: new Date(),
        auditTrail: []
      }]
    })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!threadId) {
        threadId = `tier3-${new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5)}`
        setCurrentThreadId(threadId)
      }
      ...
      const agentTypeMap = {
        'operations-recommendation': 'operations_recommendation',
        'executive-briefing': 'executive_briefing'
      }

      const response = await fetch(`${API_BASE}/api/agent/tier3/${agentTypeMap[activeTab]}`, {
        method: 'POST',
        headers: {
  ...
  const tabs = [
    { id: 'operations-recommendation', name: 'Operations Recommendation', icon: Lightbulb, color: '#1C3668' },
    { id: 'executive-briefing', name: 'Executive Briefing', icon: FileText, color: '#2E558F' }
  ]
  ...
  const quickActions = {
    'operations-recommendation': [
      { label: 'Top Actions', prompt: 'What are the top 3 actions to improve throughput?', icon: TrendingUp },
      { label: 'Maintenance Focus', prompt: 'Where should maintenance focus this week?', icon: Target },
      { label: 'Biggest Win', prompt: 'What is the biggest yield or throughput improvement available across the plant right now?', icon: Award }
    ],
    'executive-briefing': [
      { label: 'Daily Summary', prompt: 'Summarise yesterday for leadership', icon: FileText },
      { label: 'Weekly Risks', prompt: 'What should I worry about this week?', icon: Target },
      { label: 'Track Status', prompt: 'Are we on track this month?', icon: TrendingUp }
    ]
  }
      ...
      <div className="page-header" style={{ marginBottom: '12px' }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '2px' }}>
            <h1 className="page-title" style={{ fontSize: '20px', margin: 0 }}>Recommendations</h1>
          ...
          <p className="page-subtitle" style={{ margin: 0, fontSize: '11px' }}>
            AI agents for operational recommendations and executive briefings
          </p>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{
              ...
              {activeTab === 'operations-recommendation' && 'Suggests actions by consuming outputs from Analyst + RCA agents. Ranks improvement opportunities, estimates impact, and explains trade-offs to help you make informed decisions.'}
              {activeTab === 'executive-briefing' && 'Translates factory reality to leadership by creating exec-ready briefs from summaries of other agents. Highlights risks & wins while avoiding technical detail to save leadership time.'}
            </p>
          </div>
          ...
          <div style={{
            ...
            <p style={{
              ...
              {activeTab === 'operations-recommendation' && '👉 Bridges insight → decision'}
              {activeTab === 'executive-briefing' && '👉 Massive time savings for leadership'}
            </p>
          </div>
          ...
          <div style={{
            ...
            <ul style={{
              ...
              {activeTab === 'operations-recommendation' && (
                <>
                  <li>Outputs from Analyst agents</li>
                  <li>Outputs from RCA agents</li>
                  <li>Performance metrics</li>
                </>
              )}
              {activeTab === 'executive-briefing' && (
                <>
                  <li>Summaries from other agents</li>
                  <li>KPI trends</li>
          ...
          <div style={{ background: '#fff', borderRadius: '12px', width: '100%', maxWidth: '560px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '20px 24px 16px', borderBottom: '1px solid #E8ECF4' }}>
              <div>
                ...
                <div style={{ fontSize: '16px', fontWeight: '700', color: '#0A1628' }}>Recommendations</div>
              </div>
            ...
            <div style={{ overflowY: 'auto', padding: '20px 24px 24px', fontSize: '12px', color: '#374151', lineHeight: '1.6' }}>
              <div style={{ marginBottom: '16px', paddingBottom: '16px', borderBottom: '1px solid #F0F4FA' }}>
                ...
                <p style={{ margin: 0 }}>Hosts Tier 3 Predictive &amp; Prescriptive agents that orchestrate lower-tier agents to generate recommendations and executive briefings. These agents call Tier 1 and Tier 2 agents internally before synthesising a response.</p>
              </div>
              <div style={{ marginBottom: '16px', paddingBottom: '16px', borderBottom: '1px solid #F0F4FA' }}>
                ...
                <ul style={{ margin: 0, paddingLeft: '16px' }}>
                  <li style={{ marginBottom: '4px' }}><strong>Operations Recommendation</strong> — Orchestrates Tier 1 &amp; 2 agents to produce ranked improvement recommendations</li>
                  <li style={{ marginBottom: '4px' }}><strong>Executive Briefing</strong> — Produces concise plain-language summaries for plant leadership</li>
                </ul>
              </div>
              <div style={{ marginBottom: 0 }}>
                ...
                <ul style={{ margin: 0, paddingLeft: '16px' }}>
                  <li style={{ marginBottom: '4px' }}><strong>All Fabric tables</strong> — Accessed via agent-to-agent orchestration across Tier 1 and Tier 2</li>
                </ul>
              </div>
            </div>
          </div>
...
export default Tier3PredictivePrescriptiveAgents
